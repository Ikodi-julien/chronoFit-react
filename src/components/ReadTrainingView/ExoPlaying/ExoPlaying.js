...
const ExoPlaying = ({
  ...
  useEffect(() => {
    if (isTransition) setTimeout(() => setIsTransition(false), 10);
  });

  return (
    <section className="readtraining__exoplaying">
      <BipManager time={countdownTime} isCountdown={isCountdown} />
      <h1 className="readtraining__exoplaying__title">Exo en cours</h1>
      <div
        className={`readtraining__exoplaying__container ${
          isTransition && "--position-left"
        }`}
      >
        <VideoManager isCounting={exoPlaying.isCounting} />
        <h1 className="readtraining__exoplaying__name">{exoPlaying.name}</h1>
